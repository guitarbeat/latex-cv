...
## 2026-01-22 - [Script Caching]
**Learning:** Expensive data extraction tools like `pandoc` and `pdftotext` in shell scripts often run unconditionally, slowing down the dev loop. Timestamp-based caching (checking if output is newer than input with `-nt`) is a simple, dependency-free way to eliminate this overhead.
**Action:** Always wrap expensive file processing commands in `if [ ! -f out ] || [ src -nt out ];` blocks in shell scripts.
